  ...
  if (!permit)
    return (
      <div className="layout">
        <Sidebar />
        <main className="main-content">
          <div className="alert alert-danger">Permit not found.</div>
        </main>
      </div>
    );
  ...
  const sections = [
    {
      ...
      items: [
        ["Full Name", permit.applicantName],
        ["Contact Number", permit.contactNumber],
        ["Email", permit.email],
        ["Address", permit.applicantAddress],
      ],
    },
    {
      ...
      items: [
        ...
        ["Municipality", permit.municipality || "Balagtas"],
        ["Province", permit.province || "Bulacan"],
        ["Lot Number", permit.lotNumber],
        ["Block Number", permit.blockNumber],
        ["TCT/OCT No.", permit.tctNumber],
        ["Tax Dec. No.", permit.taxDeclarationNumber],
        [
          "GPS Coordinates",
          permit.latitude
    ...
    },
    {
      ...
      items: [
        ["Scope of Work", permit.scopeOfWork],
        ["Use/Occupancy", permit.useOrCharacterOfOccupancy],
        [
          "Project Cost",
          permit.projectCost
            ? `₱${Number(permit.projectCost).toLocaleString()}`
            : null,
        ],
        ["Floor Area", permit.floorArea ? `${permit.floorArea} sq.m` : null],
        ["Lot Area", permit.lotArea ? `${permit.lotArea} sq.m` : null],
        ["No. of Storeys", permit.numberOfStoreys],
        [
          "Building Height",
          permit.buildingHeight ? `${permit.buildingHeight} m` : null,
        ],
      ],
    },
    {
      ...
      items: [
        ["Architect/Engineer", permit.architectEngineerName],
        ["PRC License No.", permit.architectEngineerLicenseNo],
        ["PTR No.", permit.architectEngineerPrcNo],
        ["Contact", permit.architectEngineerContact],
        ["Electrical Engineer", permit.electricalEngineerName],
        ["EE License No.", permit.electricalEngineerLicenseNo],
        ["Sanitary Engineer", permit.sanitaryEngineerName],
        ["Mechanical Engineer", permit.mechanicalEngineerName],
      ],
    },
  ];
      ...
      <main className="main-content">
        <div className="page-header">
          <div className="breadcrumb">
            <Link to="/admin/permits" style={{ color: "var(--primary)" }}>
              ← All Applications
            </Link>{" "}
            / <span>{permit.transactionNumber}</span>
          </div>
          ...
          <div>
            ...
            >
              ...
              >
                ...
                {/* Inspection info */}
                {permit.status === "scheduled_for_inspection" &&
                  permit.inspectionDate && (
                    ...
                    >
                      ...
                      <div className="datetime">
                        {new Date(permit.inspectionDate).toLocaleDateString(
                          "en-PH",
                          {
                            weekday: "short",
                            month: "short",
                        ...
                        {permit.inspectionTime &&
                          ` at ${permit.inspectionTime}`}
                      </div>
                      {permit.inspectionTeam && (
                        <div className="sub">Team: {permit.inspectionTeam}</div>
                      )}
                      {permit.inspectionNotes && (
                ...
                {permit.assessedFee && (
                  <div>
                    ...
                    >
                      ₱{Number(permit.assessedFee).toLocaleString()}
                    </div>
                    {permit.amountPaid && (
                      <div style={{ fontSize: 11, color: "var(--success)" }}>
                        Paid: ₱{Number(permit.amountPaid).toLocaleString()} ·
                        OR#{permit.orNumber}
                      </div>
                    )}
                  </div>
            ...
            <div className="card">
              <div className="card-title" style={{ marginBottom: 12 }}>
                Submitted Documents ({permit.attachments?.length || 0})
              </div>
            ...
            {nextStatuses.length > 0 && (
              <div className="card">
                ...
                <div className="form-group">
                  ...
                  >
                    <option value={permit.status}>
                      {STATUS_LABELS[permit.status]} (current)
                    </option>
                    {nextStatuses.map((s) => (
                  ...
                  >
                    ...
                    <div className="form-group">
                      ...
                      <input
                        type="date"
                        className="form-control"
                        value={statusForm.inspectionDate}
                        onChange={(e) =>
                          setStatusForm((f) => ({
                            ...f,
                    ...
                    <div className="form-group">
                      <label className="form-label">
                        Inspection Time <span>*</span>
                      </label>
                      <input
                        type="time"
                        className="form-control"
                        value={statusForm.inspectionTime}
                        onChange={(e) =>
                          setStatusForm((f) => ({
                            ...f,
                    ...
                    <div className="form-group">
                      ...
                      <input
                        className="form-control"
                        value={statusForm.inspectionTeam}
                        onChange={(e) =>
                          setStatusForm((f) => ({
                            ...f,
                            inspectionTeam: e.target.value,
                          }))
                        }
                        placeholder="e.g., Engr. Santos & Team"
                      />
                    </div>
                    <div className="form-group" style={{ marginBottom: 0 }}>
                      <label className="form-label">
                        Notes for Inspection Team
                      </label>
                      <textarea
                        className="form-control"
                        rows={2}
                        value={statusForm.inspectionNotes}
                        onChange={(e) =>
                          setStatusForm((f) => ({
                            ...f,
                ...
                {statusForm.status === "for_payment" && (
                  <div className="form-group">
                    <label className="form-label">
                      Assessed Fee (PHP) <span>*</span>
                    </label>
                    <input
                      type="number"
                      className="form-control"
                      value={statusForm.assessedFee}
                      onChange={(e) =>
                        setStatusForm((f) => ({
                          ...f,
                          assessedFee: e.target.value,
                        }))
                      }
                      placeholder="0.00"
                    />
              ...
              permit.inspectionDate && (
                <div className="card" style={{ padding: 14 }}>
                  <div className="inspection-card">
                    ...
                    {permit.inspectionTime && (
                      <div className="sub">⏰ {permit.inspectionTime}</div>
                    )}
                    {permit.inspectionTeam && (
                      <div className="sub">👥 {permit.inspectionTeam}</div>
                    )}
                    {permit.inspectionNotes && (
                      <div
                        className="sub"
              ...
              <div className="timeline">
                {[...permit.statusHistory].reverse().map((h, i) => (
                  <div key={i} className="timeline-item">
                    <div
                      className={`timeline-dot ${i === 0 ? "latest" : h.status === "scheduled_for_inspection" ? "inspection" : ""}`}
                    />
        ...
        {showMsg && (
          ...
          >
            <div className="compose-modal" onClick={(e) => e.stopPropagation()}>
              ...
              >
                To: <strong>{permit.applicantName}</strong> — Re:{" "}
                {permit.transactionNumber}
              </div>
