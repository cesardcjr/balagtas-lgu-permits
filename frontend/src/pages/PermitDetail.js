...
export default function PermitDetail() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${process.env.REACT_APP_API_URL}/api/messages/admin-contact`)
      ...
      <main className="main-content">
        <div className="page-header">
          <div className="breadcrumb">
            <Link to="/my-permits" style={{ color: "var(--primary)" }}>
              ← My Applications
            </Link>{" "}
        ...
        <div
          className="card"
          style={{ borderLeft: "4px solid var(--primary)", marginBottom: 18 }}
        >
          <div
            style={{
              display: "flex",
          ...
          >
            ...
            {permit.assessedFee && (
              <div style={{ textAlign: "right" }}>
                ...
                <div
                  style={{ fontSize: 22, fontWeight: 800, color: "#854d0e" }}
                >
                ...
                {permit.amountPaid && (
                  ...
                  >
                    ✅ Paid: ₱{Number(permit.amountPaid).toLocaleString()} · OR#
                    {permit.orNumber}
                  </div>
                )}
              </div>
            ...
            <div className="alert alert-purple" style={{ marginBottom: 18 }}>
              ...
              <div>
                ...
                <div style={{ marginTop: 4 }}>
                  <strong>Date:</strong>{" "}
                  {new Date(permit.inspectionDate).toLocaleDateString("en-PH", {
                    weekday: "long",
                    year: "numeric",
                    month: "long",
                    day: "numeric",
                  ...
                  {permit.inspectionTime && (
                    <>
                      {" "}
                      at <strong>{permit.inspectionTime}</strong>
                    </>
                  )}
                </div>
                {permit.inspectionTeam && (
                  <div>
                    <strong>Inspection Team:</strong> {permit.inspectionTeam}
                  </div>
                )}
          ...
          >
            ...
            <div className="card">
              ...
              <InfoRow
                ...
                value={permit.propertyAddress}
              />
              <InfoRow label="Barangay" value={permit.barangay} />
              <InfoRow
                label="Municipality / Province"
                value={`${permit.municipality || "Balagtas"}, ${permit.province || "Bulacan"}`}
              />
              <InfoRow label="Lot Number" value={permit.lotNumber} />
              <InfoRow label="TCT / OCT No." value={permit.tctNumber} />
              <InfoRow
                label="Tax Declaration No."
                value={permit.taxDeclarationNumber}
              />
              {permit.latitude && (
                <InfoRow
                  label="GPS Coordinates"
                  value={`${parseFloat(permit.latitude).toFixed(6)}, ${parseFloat(permit.longitude).toFixed(6)}`}
                />
              )}
            </div>
            ...
            <div className="card">
              ...
              <InfoRow
                label="Use / Occupancy"
                value={permit.useOrCharacterOfOccupancy}
              />
              <InfoRow
                label="Project Cost"
                value={
              ...
              <InfoRow
                label="Floor Area"
                value={permit.floorArea ? `${permit.floorArea} sq.m` : null}
              />
              <InfoRow
                label="Number of Storeys"
                value={permit.numberOfStoreys}
              />
              <InfoRow
              ...
              <InfoRow
                ...
                value={permit.architectEngineerName}
              />
            </div>
          </div>
          ...
          >
            ...
            <div className="card">
              <div className="card-title" style={{ marginBottom: 12 }}>
                Attached Documents ({permit.attachments?.length || 0})
              </div>
        ...
        {showCompose && (
          ...
          >
            <div className="compose-modal" onClick={(e) => e.stopPropagation()}>
              ...
              >
                Re: {permit.transactionNumber} —{" "}
                {PERMIT_LABELS[permit.permitType]}
              </div>
