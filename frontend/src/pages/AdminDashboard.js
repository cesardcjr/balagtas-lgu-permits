...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      axios.get(`${process.env.REACT_APP_API_URL}/api/permits/stats/summary`),
      axios.get(`${process.env.REACT_APP_API_URL}/api/permits/all`),
    ])
      .then(([s, p]) => {
        setStats(s.data);
          ...
          <>
            ...
            >
              ...
              >
                <div className="card-header">
                  ...
                  <Link to="/admin/permits" className="btn btn-outline btn-sm">
                    View All →
                  </Link>
                </div>
                    ...
                    <tbody>
                      {recent.map((p) => (
                        <tr key={p._id}>
                          ...
                          <td>
                            <span
                              className={`permit-type-tag tag-${p.permitType}`}
                              style={{ fontSize: 9 }}
                            >
            ...
            <div className="card">
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                <Link
                  to="/admin/permits?status=pending"
                  className="btn btn-outline btn-sm"
                >
                  ⏳ Pending ({stats?.pending || 0})
                </Link>
                <Link
                  to="/admin/permits?status=under_review"
                  className="btn btn-outline btn-sm"
                >
                  🔍 Under Review
                </Link>
                <Link
                  to="/admin/permits?status=scheduled_for_inspection"
                  className="btn btn-purple btn-sm"
                >
                  📅 For Inspection ({stats?.scheduled || 0})
                </Link>
                <Link
                  to="/admin/permits?status=for_payment"
                  className="btn btn-outline btn-sm"
                >
                  💳 For Payment
                </Link>
                <Link
                  to="/admin/permits?status=approved"
                  className="btn btn-success btn-sm"
                >
                  ✅ Approved / Release
                </Link>
                <Link to="/admin/inbox" className="btn btn-primary btn-sm">
                  ✉️ Inbox
                </Link>
              </div>
            </div>
